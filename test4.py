...
# setup yolov5
model = attempt_load("/home/pi/venv/last.pt", map_location="cpu")  # load FP32 model
...
def detect(img):
	...
	pred = model(img)[0]
	pred = non_max_suppression(pred,float(0.5), float(0.45))
...
def send_mail():
	try:
		...
		message = """\
		Subject: WARNING: TRASH CAN IS FULL\n"""+"\n"+"So luong chai nhua: "+str(score_bottle)+"\nSo qua 5 chai da phat: "+str(score_gift1)+"\
...
while True:
	...
	if (button1=="On") and (score>=10):
        for i in range(0,int(score//10)):
            servo2()
            score_gift2+=1
        score=0
	print(score_gift1,"//////",score_gift2)
